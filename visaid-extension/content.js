...
function normalizeNumberWords(text) {
    ...
    const digits = words.map(w => map[w] ?? w);

    return digits.join("").replace(/\D/g,"");
}
...
function startListening() {
    ...
    recognition.onresult = (event) => {
        ...
        if (currentIndex >= fields.length) {
            ...
            if (speech.includes("yes")) submitForm();
            else speak("Form submission cancelled");
        ...
        const isEmail = field.type==="email" || field.name?.toLowerCase().includes("email");
        ...
        else if (isPhone) transcript = normalizeNumberWords(transcript);
...
function fillField(fieldName, value) {
    ...
    if (select) {
        ...
        let match = options.find(o => spoken === o.text.toLowerCase());
